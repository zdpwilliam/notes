...
def load_lesson_stu_id_list(conn_tr, lesson_id_list):
    sql_forge = "SELECT id, tea_id FROM lessons WHERE id in (%s -1)"
    sql_ids = ""
...
def update_format_stu_id(lesson_stu_id_tuple_list):
    conn_tr_inner = MySQLdb.connect(host="rm-bp1c76k6t475l4m99.mysql.rds.aliyuncs.com", user="tr_sa",
                                    passwd="8CXwb17Lz5AlML2V", db='tr', charset="utf8")
    sql_tr = "update lesson_course_system set tea_id = %s WHERE lesson_id = %s"
...
batch_size = 200
# prepare for lessonId and teaId map
try:
