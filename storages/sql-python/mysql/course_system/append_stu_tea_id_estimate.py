...
def load_lesson_id_list(conn_tr):
    sql_forge = "SELECT lesson_id FROM lesson_test_estimate GROUP BY lesson_id"
...
def load_lesson_stu_id_list(conn_tr, lesson_id_list):
    sql_forge = "SELECT id, stu_id, tea_id FROM lessons WHERE id in (%s -1)"
    sql_ids = ""
...
def update_format_stu_id(lesson_stu_id_tuple_list):
    ...
    sql_tr = "update lesson_test_estimate set stu_id = %s, tea_id = %s WHERE lesson_id = %s "
    cursor = conn_tr_inner.cursor()
    counter = 0
    for lesson_stu_id in lesson_stu_id_tuple_list:
        print sql_tr % (lesson_stu_id[1], lesson_stu_id[2], lesson_stu_id[0])
        counter += cursor.execute(sql_tr, (lesson_stu_id[1], lesson_stu_id[2], lesson_stu_id[0]))
